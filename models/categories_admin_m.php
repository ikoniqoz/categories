...
class Categories_admin_m extends MY_Model
{
	...
	private function getNewSlug($inSlug='', $igNor = -1)
	{
		...
		$suffix = (isset($category)) ? '-'. $this->db->where('id !=',$igNor)->like('slug', $category->slug)->get('shop_categories')->num_rows() : '' ;
    ...
    public function delete($id)
    {
        ...
        //
        $this->db->where('category_id', $id )->delete('shop_categories_products');
	...
	public function get_products_filter($i_array)
	{
		...
		{
			$i_array["By Categories"]["shop_categories,{$value->id}|{$value->id}"] = $value->name;
		}
