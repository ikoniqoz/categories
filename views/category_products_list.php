...
{{ endif}} 
    ...
    {{if category:parent_id > 0 }}
        {{shop_categories:category id="{{category:parent_id}}" }}
            <a class='btn btn-sm'  href="{{url:site}}shop/categories/category/{{slug}}">&larr; back to  {{name}}</a>  |
        {{/shop_categories:category}}
    {{endif}}

    
    <!--  Static link for consistency -->        
    <a  class='btn btn-sm'  href="{{url:site}}shop/categories/">all categories</a>     


    <!--  Helpful resource lin kto cart -->
    {{ shop:cart }}
        {{if item_count > 0}}
            | <a  class='btn btn-sm'  href="{{url:site}}shop/cart/">view cart</a>    
        {{ endif }}
    {{ /shop:cart }}
        ...
        {{ if category:product_count > 3 }}
            | <a class='btn btn-sm' href="{{url:site}}shop/categories/products/{{category:slug}}">discover {{category:product_count}} more products in this category &rarr;</a>  
        {{ endif }}
